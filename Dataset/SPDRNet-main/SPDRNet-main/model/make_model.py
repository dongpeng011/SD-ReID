...
"""
@author:  Dong Peng
@contact: 847704826@qq.com
"""
import torch
import torch.nn as nn
import copy
from model.backbones.vit_pytorch import vit_base_patch16_224_ISGA
...
class BuildTransformer(nn.Module):
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None, dataset_label=None):
        ...
        global_feat = self.base(x, camera_id=cam_label, view_id=view_label)
...
class BuildMultiSceneTransformer(nn.Module):
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None, dataset_label=None, forward_aux=False):

        if self.training:
            gt_feat = self.base(x, camera_id=cam_label, view_id=view_label)
            bn_feat = self.bottleneck(gt_feat)
            fc_feat = self.classifier(bn_feat)
            if self.aux_loss and forward_aux:
                # 兼容旧训练代码的 auxiliary loss 格式，但使用统一特征
                return gt_feat, fc_feat, gt_feat
            else:
                return gt_feat, fc_feat
        else:
            feat = self.base(x, camera_id=cam_label, view_id=view_label)
...
__factory_T_type = {
    'vit_base_patch16_224_ISGA': vit_base_patch16_224_ISGA,
    'vit_base_patch16_224_TransReID': vit_base_patch16_224_ISGA,  # 兼容你原本的配置文件
}
...
def make_model(cfg, num_class, camera_num, view_num):
    if cfg.MODEL.TYPE == 'TR':
        if cfg.MODEL.MULTI_SCENES:
            model = BuildMultiSceneTransformer(num_class, camera_num, view_num, cfg, __factory_T_type)
            print('===========Build Multi-scene ISGA-ViT===========')
        else:
            model = BuildTransformer(num_class, camera_num, view_num, cfg, __factory_T_type)
            print('===========Build ISGA-ViT===========')
    else:
        raise NotImplementedError

    if cfg.MODEL.EMA:
        model = EMA(model, cfg.MODEL.EMA_M)
        print('===========EMA Model===========')
    return model